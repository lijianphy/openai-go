...
// main runs an interactive DASHSCOPE chatbot session on stdin/stdout.
func main() {
	...
	if baseURL == "" {
		...
		fmt.Printf("Using custom DASHSCOPE API base URL: %s\n", baseURL)
	}
	if apiKey == "" {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "failed to open DASHSCOPE debug log file: %v\n", err)
		os.Exit(1)
	}
	defer func() {
		if err := debugLogFile.Close(); err != nil {
			fmt.Fprintf(os.Stderr, "failed to close DASHSCOPE debug log file: %v\n", err)
		}
	}()
	...
	fmt.Printf("Simple DASHSCOPE chatbot started with model %s\n", model)
	fmt.Printf("DASHSCOPE debug log: %s\n", debugLogPath)
...
func formatError(err error) string {
	...
	if errors.As(err, &apiErr) && apiErr.Message != "" {
		return fmt.Sprintf("DASHSCOPE API error (%d): %s", apiErr.StatusCode, apiErr.Message)
	}
...
func openDebugLogFile() (*os.File, string, error) {
	...
	path := filepath.Join("debug_log", fmt.Sprintf("dashscope-%s.log", timestamp))
